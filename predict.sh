...
PYTHONPATH=src_legacy python src_legacy/scripts/athene/doc_retrieval_athene.py --database none --infile $1 --outfile tmp/ir.$(basename $1) --path_wiki_titles $path_wiki_title

echo "starting sentence retrieval part 1"

python src_legacy/domlin/sentence_retrieval_part_1.py --infile tmp/ir.$(basename $1) --outfile tmp/sentences_1.$(basename $1) --path_wiki_titles $path_wiki_title


CUDA_VISIBLE_DEVICES=0 python src_legacy/domlin/run_fever.py --task_name=ir --do_train=false --do_eval=false --do_predict=true \
...
python src_legacy/domlin/sentence_retrieval_part_2.py --infile tmp/ir.$(basename $1) --outfile tmp/sentences_2.$(basename $1) --path_wiki_titles $path_wiki_title --file_with_sentences_to_be_predicted  tmp/sentences_1.$(basename $1) --predicted_evidence tmp/sentences_1_predicted.$(basename $1)


python src_legacy/domlin/run_fever.py --task_name=combined_evidence --do_train=false --do_eval=false --do_predict=true \
...
python src_legacy/domlin/generate_rte.py --infile tmp/ir.$(basename $1) --outfile tmp/rte.$(basename $1) \
...
python src_legacy/domlin/run_fever.py --task_name=fever --do_train=false --do_eval=false --do_predict=true \
--vocab_file=cased_L-12_H-768_A-12/vocab.txt --bert_config_file=cased_L-12_H-768_A-12/bert_config.json \
...
python src_legacy/domlin/generate_submission.py --path_rte_file tmp/rte.$(basename $1) --path_rte_predictions tmp/rte_predicted.$(basename $1) --infile=tmp/ir.$(basename $1) \
