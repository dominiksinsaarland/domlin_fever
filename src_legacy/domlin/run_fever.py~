...
tf.flags.DEFINE_string("path_to_train_file", None, "[Optional] TensorFlow master URL.")
...
class CombinedEvidence(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    lines = self._read_tsv(FLAGS.path_to_train_file)
    examples = []
  ...
  def get_dev_examples(self, data_dir):
    ...
    if FLAGS.prediction_file is not None:
      ...
      lines = self._read_tsv(os.path.join(data_dir, "combined_evidence_dev_set.tsv"))
...
class DocumentRetrieval(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    if FLAGS.path_to_train_file is not None:
      lines = self._read_tsv(FLAGS.path_to_train_file)
    else:
...
class FEVERProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    if FLAGS.train_file is None:
      lines = self._read_tsv(os.path.join(data_dir, "RTE_train_set.tsv"))
    else:
      lines = self._read_tsv(FLAGS.train_file)
