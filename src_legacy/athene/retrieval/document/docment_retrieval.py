...
class Doc_Retrieval:
    ...
    def __init__(self, database_path, add_claim=False, k_wiki_results=None,path_wiki_titles=None):
        ...
        self.wiki_titles = self.read_wiki_pages(path_wiki_titles)

    def read_wiki_pages(self, path):
        wiki_titles = set()
...
def main(db_file, k_wiki, in_file, out_file, path_wiki_titles, add_claim=True, parallel=True):
    # tfidf_path = "data/index/fever-tfidf-ngram=2-hash=16777216-tokenizer=simple.npz"
    method = Doc_Retrieval(database_path=db_file, add_claim=add_claim, k_wiki_results=k_wiki, path_wiki_titles=path_wiki_titles)
    processed = dict()
    path = os.getcwd()
    jlr = JSONLineReader()
